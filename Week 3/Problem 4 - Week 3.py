...
# 2. 캐릭터의 바로 왼쪽 방햐에 아직 가보지 않은 칸이 존재한다면, 왼쪽 방향으로 회전한 다음 왼쪽으로 한 칸을 전진한다.
...
# 입력 : 첫째 줄에 맵으 ㅣ세로 크기 N과 가로 크기 M을 공백으로 구분하여 입력한다. (3<=N, M <= 50)
...
# 출력 : 첫째 줄에 이동을 마친 후 캐릭터가 방문한 칸의 수를 출력한다.
...
# idea : 플레이어가 밟은 육지는 2로 표시하기.

mapping[x][y] = 2

while True:

    if mapping[x+dx[0]][y+dy[0]]!=0 and mapping[x+dx[1]][y+dy[1]]!=0 and mapping[x + dx[2]][y + dy[2]] != 0\
            and mapping[x+dx[3]][y+dy[3]]!=0:
