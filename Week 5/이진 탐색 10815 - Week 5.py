...
def checking(card, target, start, end):
    ...
    if card[mid]==target:
        ...
        return(checking(card, target, start, end=mid-1))
    else:
        return(checking(card, target, start=mid+1, end=end))

    return(0)


for i in check:
    print(checking(card=card, target=i, start=0, end=len(card)-1), end=' ')